...
ChatLogic::ChatLogic()
{
    //// STUDENT CODE
    ////
    // we remove all of this as we use unique pointer
...
ChatLogic::~ChatLogic()
{
    //// STUDENT CODE
    ////
    //remove also as we use unique pointer
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> & node ) { return node->GetID() == id; });                        // create new element if ID does not yet exist
                    ...
                    if (type->second == "EDGE")
                    {
                        ...
                        if (parentToken != tokens.end() && childToken != tokens.end())
                        {
                            ...
                            // create new edge
                            //using the unique-ptr to make placein Heap for GraphEdge
                            std::unique_ptr<GraphEdge> edge = std::make_unique<GraphEdge>(id);                            //modify using get()
                            edge->SetChildNode((*childNode).get());
                            edge->SetParentNode((*parentNode).get());
                            //_edges.push_back(edge);  not used now
    ...
    // create an object of in graph root node
    ChatBot chatBotobj("../images/chatbot.png");
    //
    chatBotobj.SetChatLogicHandle(this);
    //
    chatBotobj.SetRootNode(rootNode);
    //
    _chatBot = &chatBotobj;
    //
    rootNode->MoveChatbotHere(std::move(chatBotobj));
